//nvcc OneSidedJacobiParallelGlobal.cu svd_one_sided_jacobi_C.cu host_functions.cu -o OneSidedJacobiParallelGlobal
...
int main (int argc, char * argv[]) {
	...
	*/
	...
	sprintf(fileName, "SingularValues/CudaDevice/OneSidedParallelGlobal/Singular Values Cuda Device %dx%d.txt", rows, cols);
	openFile(&fp, fileName, "w");
	...
	sprintf(fileName, "Time/CudaDevice/OneSidedParallelGlobal/Time %dx%d.txt", rows, cols);
	openFile(&fp, fileName, "w");
